...
SYSUCC_MEAN = (0.65384524, 0.34809846, 0.28892109)       # SYSUCC RGB VALUES
SYSUCC_STD = (0.18437574, 0.16062852, 0.13951364)        # SYSUCC RGB VALUES
...
""""""""""""""""""""""""""""""""""""""""""
"""" FUNCTIONS FOR SYSUCC DATASET """
""""""""""""""""""""""""""""""""""""""""""


def read_inclusion_sysucc(path, criteria):
    ...
    for k_cache, v_cache in cache.items():
        ...
        # Check whether include is true
        if include:
            if v_cache['class'] == 'Cancer':
                info = {'file': v_cache['file'], 'label': np.array([1], dtype=np.float32), 'roi': v_cache['roi']}
                img_list.append(info)
            elif v_cache['class'] == 'Normal':
                info = {'file': v_cache['file'], 'label': np.array([0], dtype=np.float32), 'roi': v_cache['roi']}
                img_list.append(info)
            else:
                print('Unrecognized class..')
                raise ValueError
...
def sample_weights_sysucc(img_list):
...
def augmentations_sysucc(opt):
    ...
    # Specify augmentation techniques for training
    train_technique1 = random.choice([Identity_SYSUCC(),
                                      Identity_SYSUCC(),
                                      GaussianBlur_SYSUCC(),
                                      RandomAdjustSharpness_SYSUCC(sharpness_factor=2, p=1),
                                      RandomAffine_SYSUCC(max_rotate=25, max_translate=5, max_shear=15)])
    train_technique2 = random.choice([Resize_SYSUCC([opt.imagesize, opt.imagesize]),
                                      RandomResizedCrop_SYSUCC((opt.imagesize, opt.imagesize)),
                                      RandomResizedCrop_SYSUCC((opt.imagesize, opt.imagesize), scale=(0.7, 1.1))])
    train_technique3 = random.choice([Identity_SYSUCC(),
                                      Grayscale_SYSUCC(num_output_channels=3),
                                      ColorJitter_SYSUCC(brightness=0.5, contrast=0.5, saturation=0.5, hue=0.0),
                                      ColorJitter_SYSUCC(brightness=0.3, contrast=0.3, saturation=0.3, hue=0.0),
                                      ColorJitter_SYSUCC(brightness=0.1, contrast=0.1, saturation=0.1, hue=0.0)])
    train_transforms.extend([train_technique1,
                             train_technique2,
                             train_technique3,
                             RandomHorizontalFlip_SYSUCC(p=0.5),
                             RandomVerticalFlip_SYSUCC(p=0.5),
                             Rotate_SYSUCC([0, 90, 180, 270, 360]),
                             ToTensor_SYSUCC(),
                             Normalize_SYSUCC(mean=[SYSUCC_MEAN[0], SYSUCC_MEAN[1], SYSUCC_MEAN[2]],
                                              std=[SYSUCC_STD[0], SYSUCC_STD[1], SYSUCC_STD[2]])])

    # Specify augmentation techniques for validation set
    val_technique1 = random.choice([Resize_SYSUCC([opt.imagesize, opt.imagesize]),
                                    RandomResizedCrop_SYSUCC((opt.imagesize, opt.imagesize))])
    val_technique2 = random.choice([Identity_SYSUCC(),
                                    ColorJitter_SYSUCC(brightness=0.3, contrast=0.3, saturation=0.3, hue=0.0),
                                    ColorJitter_SYSUCC(brightness=0.1, contrast=0.1, saturation=0.1, hue=0.0)])
    val_transforms.extend([val_technique1,
                           val_technique2,
    ...
    # Specify augmentation techniques for test set
    test_transforms.extend([Resize_SYSUCC([opt.imagesize, opt.imagesize]),
                            ToTensor_SYSUCC(),
                            Normalize_SYSUCC(mean=[SYSUCC_MEAN[0], SYSUCC_MEAN[1], SYSUCC_MEAN[2]],
                                             std=[SYSUCC_STD[0], SYSUCC_STD[1], SYSUCC_STD[2]])])

    # Compose transforms and place into dictionary
    data_transforms['train'] = Compose_SYSUCC(train_transforms)
    data_transforms['val'] = Compose_SYSUCC(val_transforms)
    data_transforms['test'] = Compose_SYSUCC(test_transforms)

    return data_transforms


class DATASET_TRAIN_TEST_SYSUCC(Dataset):
    def __init__(self, inclusion, transform=None, random_noise=False):
        self.inclusion = inclusion
        self.transform = transform
        self.random_noise = random_noise
...
class DATASET_VAL_SYSUCC(Dataset):
    def __init__(self, inclusion, transform=None):
...
# Custom Horizontal Flipping
class RandomHorizontalFlip_SYSUCC:
    def __init__(self, p=0.5):
        self.p = p
...
# Custom Vertical Flipping
class RandomVerticalFlip_SYSUCC:
    def __init__(self, p=0.5):
        self.p = p
...
# Custom Random Adjusting Sharpness
class RandomAdjustSharpness_SYSUCC:
    def __init__(self, sharpness_factor, p=1.):
        self.sharpness = transforms.RandomAdjustSharpness(sharpness_factor=sharpness_factor, p=p)
...
# Custom Class for Gaussian Blurring images
class GaussianBlur_SYSUCC(object):
    """
    Apply Gaussian Blur to the PIL image.
    """
...
# Custom Class for making random resized crops for input images, and resize to desired size afterwards
class RandomResizedCrop_SYSUCC(object):
    """Crop the given PIL Image to random size and aspect ratio.
