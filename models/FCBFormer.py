    ...
    def __init__(self, version):
        ...
        for i in range(4):
            ...
            if option == 'PVT-V2-B0':
                self.LE.append(
                    nn.Sequential(
                        RB([32, 64, 160, 256][i], 64), RB(64, 64), nn.Upsample(size=88)
                    )
                )
            else:
                self.LE.append(
...
class FCB(nn.Module):
    def __init__(self, in_channels=3, min_level_channels=32, min_channel_mults=[1, 1, 2, 2, 4, 4], n_levels_down=6,
                 n_levels_up=6, n_RBs=2, in_resolution=352, ):
