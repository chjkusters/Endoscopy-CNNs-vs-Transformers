...
""""""""""""""""""""""""""""""""""""
"""" DEFINE CARANET MODEL"""
""""""""""""""""""""""""""""""""""""
# Adapted from https://github.com/AngeLouCN/CaraNet


class CaraNet(nn.Module):
    def __init__(self, channel=32, inference=False, num_classes=1):
        super().__init__()
        ...
        # ---- ResNet Backbone ----
        self.resnet = res2net101_v1b_26w_4s(pretrained=True, num_classes=num_classes)
        ...
        # Partial Decoder
        self.agg1 = aggregation(channel, self.inference)
    ...
    def forward(self, x):
        ...
        if self.inference:
            ...
            lateral_map_1 = F.interpolate(decoder_1, scale_factor=8, mode='nearest-exact')

        # ------------------- atten-one -----------------------
        if self.inference:
            decoder_2 = F.interpolate(decoder_1, scale_factor=0.25, mode='bilinear')
        else:
            decoder_2 = F.interpolate(decoder_1, scale_factor=0.25, mode='nearest-exact')
        ...
        if self.inference:
            ...
            lateral_map_2 = F.interpolate(x_3, scale_factor=32, mode='nearest-exact')

        # ------------------- atten-two -----------------------
        if self.inference:
            decoder_3 = F.interpolate(x_3, scale_factor=2, mode='bilinear')
        else:
            decoder_3 = F.interpolate(x_3, scale_factor=2, mode='nearest-exact')
        ...
        if self.inference:
            ...
            lateral_map_3 = F.interpolate(x_2, scale_factor=16, mode='nearest-exact')

        # ------------------- atten-three -----------------------
        if self.inference:
            decoder_4 = F.interpolate(x_2, scale_factor=2, mode='bilinear')
        else:
            decoder_4 = F.interpolate(x_2, scale_factor=2, mode='nearest-exact')
        ...
        x_1 = ra_1 + decoder_4
        if self.inference:
            seg = F.interpolate(x_1, scale_factor=8, mode='bilinear')
        else:
            seg = F.interpolate(x_1, scale_factor=8, mode='nearest-exact')

        # ------------------ Classification Head ------------------------
        cls = self.resnet.avgpool(high_level_features)
        cls = cls.view(cls.size(0), -1)
        cls = self.resnet.fc(cls)

        # # For deep supervision of multiple lateral maps
        # return cls, seg, lateral_map_3, lateral_map_2, lateral_map_1

        # For deep supervision of only 1 lateral map
        return cls, seg
...
class aggregation(nn.Module):
    ...
    # used after MSF
    def __init__(self, channel, inference):
...
def res2net152_v1b_26w_4s(pretrained=False, **kwargs):
    ...
    if pretrained:
        pretrain_dict = model_zoo.load_url(model_urls['res2net152_v1b_26w_4s'])
        pretrain_dict_keys = list(pretrain_dict.keys())
        for key in pretrain_dict_keys:
            if 'fc' in key:
                del pretrain_dict[key]
